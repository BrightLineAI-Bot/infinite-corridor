...
test("release 84 loads one coherent version across the entire module graph",()=>{
  const html=readFileSync(new URL("../index.html",import.meta.url),"utf8"),sw=readFileSync(new URL("../sw.js",import.meta.url),"utf8");
  const build=readFileSync(new URL("../scripts/build.mjs",import.meta.url),"utf8");
  assert.match(html,/const release = "84"/);assert.match(html,/styles\.css\?v=84/);assert.match(html,/sw\.js\?v=\$\{release\}/);assert.match(html,/main\.js\?v=84/);assert.match(html,/controllerchange/);
  assert.match(sw,/infinite-corridor-v84/);assert.match(sw,/styles\.css\?v=84/);assert.match(sw,/main\.js\?v=84/);assert.match(sw,/combat\.js\?v=84/);assert.match(sw,/renderer\.js\?v=84/);
  assert.match(build,/release='84'/);assert.match(build,/\.js\?v=\$\{release\}/);
});
