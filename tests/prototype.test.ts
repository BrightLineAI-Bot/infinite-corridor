...
import { serializeSave, deserializeSave, normalizeSettings, effectiveQuality, journeyMetadata, createSlotWriteCoordinator, slotKey, createExclusiveOperation, commitNewJourneyTransition } from "../src/persistence.ts";
...
test("new journey transition saves only the prior occupied journey and activates after durable creation",async()=>{const calls=[],activate=[],create=async(slot,options)=>{calls.push(["create",slot,options.name]);return{slot}},saveCurrent=async()=>calls.push(["save-current"]),flush=async()=>calls.push(["flush"]);await commitNewJourneyTransition({activeSlot:1,targetSlot:2,targetEmpty:true,options:{name:"Second"},saveCurrent,flush,create,activate:slot=>activate.push(slot)});assert.deepEqual(calls,[["save-current"],["flush"],["create",2,"Second"]]);assert.deepEqual(activate,[2]);calls.length=0;activate.length=0;await commitNewJourneyTransition({activeSlot:2,targetSlot:2,targetEmpty:true,options:{name:"Fresh"},saveCurrent,flush,create,activate:slot=>activate.push(slot)});assert.deepEqual(calls,[["flush"],["create",2,"Fresh"]]);assert.deepEqual(activate,[2])});
test("failed journey persistence never changes the active slot",async()=>{let active=1;await assert.rejects(()=>commitNewJourneyTransition({activeSlot:1,targetSlot:2,targetEmpty:true,options:{},saveCurrent:async()=>{},flush:async()=>{},create:async()=>{throw new Error("storage denied")},activate:slot=>{active=slot}}),/storage denied/);assert.equal(active,1)});
test("rapid journey creation is single-flight and releases after completion",async()=>{const gate=createExclusiveOperation();let release;const held=new Promise(resolve=>{release=resolve}),first=gate.run(async()=>{await held;return"created"}),second=await gate.run(async()=>"duplicate");assert.equal(second.busy,true);release();assert.deepEqual(await first,{ok:true,value:"created"});assert.equal(gate.busy,false)});
test("journey creation and rename use in-panel forms without native prompt or stale-slot overwrite",()=>{const main=readFileSync(new URL("../src/main.ts",import.meta.url),"utf8");assert.match(main,/function renderJourneyCreator/);assert.match(main,/function renderJourneyRename/);assert.match(main,/commitNewJourneyTransition/);assert.match(main,/targetEmpty:meta\.empty/);assert.doesNotMatch(main,/prompt\(/);assert.match(main,/confirmInput\.onchange/);assert.match(main,/New journey was not created/);assert.match(main,/Journey switch failed/)});
...
test("save schema 14 preserves dungeon discovery and schema-12 progress",()=>{
...
test("Viewport selects and persists all four restrained hunt destination modes",()=>{const s=freshSave();for(const id of Object.keys(s.viewport.contracts))acceptViewportHunt(s,id);assert.equal(selectHuntDestination(s,"viewport-direct-vesperwing").kind,"ordinary-overworld");assert.equal(selectHuntDestination(s,"viewport-tracking-lattice").kind,"existing-dungeon");assert.equal(selectHuntDestination(s,"viewport-foundry-trial").kind,"temporary-arena");assert.equal(selectHuntDestination(s,"viewport-frontier-rootbound").kind,"persistent-domain");const copy=migrateSave(JSON.parse(JSON.stringify(s)));assert.deepEqual(copy.viewport.contracts["viewport-tracking-lattice"].destination,s.viewport.contracts["viewport-tracking-lattice"].destination);assert.equal(huntIntegrationDiagnostics(copy).destinations.length,5)});
test("ordinary hunt suitability rejects protected or obstructed sites and records deterministic fallback",()=>{const q={id:"hunt-safe"},open={width:12,height:12,tiles:Array.from({length:144},(_,i)=>({x:i%12,y:Math.floor(i/12),kind:"ash",blocked:false})),objects:[]};assert.equal(ordinaryHuntSuitability(open,q).ok,true);assert.equal(ordinaryHuntSuitability({...open,settlement:{id:"safe"}},q).reason,"protected-settlement");const s=freshSave();s.viewport.contracts["viewport-direct-vesperwing"].target={rx:9,ry:9};selectHuntDestination(s,"viewport-direct-vesperwing");const blocked={width:12,height:12,tiles:open.tiles.map(t=>({...t,blocked:true})),objects:[]},d=resolveHuntDestination(s,"viewport-direct-vesperwing",blocked);assert.equal(d.kind,"temporary-arena");assert.equal(d.fallbackReason,"no-safe-combat-clearing")});
test("temporary hunt uses a bespoke arena and death recovers on its valid entry",()=>{const s=freshSave();acceptViewportHunt(s,"viewport-foundry-trial");const q=s.viewport.contracts["viewport-foundry-trial"],g=new Game(s,0);g.rx=q.target.rx;g.ry=q.target.ry;g.loadArea("overworld",false);const gate=g.map.objects.find(o=>o.kind==="huntInstance");assert.ok(gate);g.player.x=gate.x;g.player.y=gate.y;assert.equal(g.interact(null,gate.id).ok,true);assert.equal(g.map.arena,true);assert.equal(g.map.arenaKind,"temporary");g.player.hp=0;g.recoverFromDeath(100);assert.equal(g.player.x,g.map.entry.x);assert.equal(g.player.y,g.map.entry.y)});
test("rare persistent arenas expose stable entrances and remain cleared on revisit",()=>{let region,entrance;for(let y=-60;!entrance&&y<=60;y++)for(let x=-60;!entrance&&x<=60;x++){if(!arenaEligible("persistent-arena-audit",x,y,"persistent"))continue;const q=generateRegion("persistent-arena-audit",x,y,1),o=q.objects.find(v=>v.kind==="arenaEntrance");if(o){region=q;entrance=o}}assert.ok(entrance);const again=generateRegion("persistent-arena-audit",region.rx,region.ry,1).objects.find(v=>v.kind==="arenaEntrance");assert.deepEqual(again,entrance);assert.ok(sectionSites("persistent-arena-audit",region.rx,region.ry,1).some(o=>o.kind==="arenaEntrance"));const arena=generateBespokeArena("persistent-arena-audit",entrance.dungeonId,{kind:"persistent"});assert.equal(arena.diagnostic.reachable,true)});
...
test("standalone Proving Ground PWA has isolated identity scope cache and diagnostics",()=>{const html=readFileSync(new URL("../proving-ground/index.html",import.meta.url),"utf8"),manifest=JSON.parse(readFileSync(new URL("../proving-ground/manifest.webmanifest",import.meta.url),"utf8")),worker=readFileSync(new URL("../proving-ground/sw.js",import.meta.url),"utf8"),mobile=readFileSync(new URL("../src/proving-ground-mobile.ts",import.meta.url),"utf8"),productionWorker=readFileSync(new URL("../sw.js",import.meta.url),"utf8");assert.equal(manifest.name,"Infinite Corridor — Proving Ground");assert.equal(manifest.scope,"./");assert.match(manifest.start_url,/dev=proving-ground/);assert.match(html,/proving-ground-mobile\.js/);assert.match(html,/Experimental · isolated scratch state/);assert.match(worker,/PREFIX="ic-proving-ground-"/);assert.doesNotMatch(worker,/CACHE_PREFIX = "infinite-corridor-"/);assert.doesNotMatch(productionWorker,/ic-proving-ground-/);assert.match(mobile,/ic-proving-ground-scratch-v1/);assert.match(mobile,/Reset only Proving Ground scratch data/);assert.doesNotMatch(mobile,/loadSave|saveGame|local-save/)});

test("production and Proving Ground manifests and workers remain separate build outputs",()=>{const build=readFileSync(new URL("../scripts/build.mjs",import.meta.url),"utf8"),productionManifest=JSON.parse(readFileSync(new URL("../manifest.webmanifest",import.meta.url),"utf8")),labManifest=JSON.parse(readFileSync(new URL("../proving-ground/manifest.webmanifest",import.meta.url),"utf8"));assert.notEqual(productionManifest.name,labManifest.name);assert.notEqual(productionManifest.start_url,labManifest.start_url);assert.match(build,/proving-ground-mobile/);assert.match(build,/proving-ground\/\$\{f\}/);assert.match(build,/PG-1/)});
