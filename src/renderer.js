...
function shack(ctx,o,s,p,map){
  const b=o.bounds||{x:o.x-2,y:o.y-2,w:5,h:4},inside=p.x>=b.x+1&&p.x<b.x+b.w-1&&p.y>=b.y+1&&p.y<b.y+b.h-1,x=b.x*s,y=b.y*s,w=b.w*s,h=b.h*s,accent={pilgrim:"#b69361",relay:"#63a7a9",chapel:"#9b7890",workshop:"#b66f50"}[o.facadeStyle]||"#b69361";
  ctx.save();if(inside){ctx.strokeStyle=accent+"88";ctx.lineWidth=2;ctx.strokeRect(x+s*.08,y+s*.08,w-s*.16,h-s*.16);ctx.restore();return}ctx.fillStyle="#292722";ctx.fillRect(x,y+s*.3,w,h-s*.3);ctx.fillStyle="#3a352d";for(let q=s*.35;q<h;q+=s*.55)ctx.fillRect(x,y+q,w,s*.1);ctx.strokeStyle=accent;ctx.lineWidth=3;ctx.strokeRect(x+s*.06,y+s*.3,w-s*.12,h-s*.36);
  ctx.beginPath();if(o.roofProfile==="shed"){ctx.moveTo(x-s*.12,y+s*.35);ctx.lineTo(x+w+s*.12,y-s*.15);ctx.lineTo(x+w+s*.12,y+s*.55);ctx.lineTo(x-s*.12,y+s*.55)}else if(o.roofProfile==="stepped"){ctx.moveTo(x-s*.12,y+s*.5);ctx.lineTo(x+w*.25,y+s*.5);ctx.lineTo(x+w*.25,y-s*.05);ctx.lineTo(x+w*.75,y-s*.05);ctx.lineTo(x+w*.75,y+s*.5);ctx.lineTo(x+w+s*.12,y+s*.5)}else{ctx.moveTo(x-s*.12,y+s*.48);ctx.lineTo(x+w*.5,y-s*.62);ctx.lineTo(x+w+s*.12,y+s*.48)}ctx.lineTo(x+w,y+s*.78);ctx.lineTo(x,y+s*.78);ctx.closePath();ctx.fillStyle="#211f1b";ctx.fill();ctx.stroke();
  const door=o.door||{x:b.x+Math.floor(b.w/2),y:b.y+b.h-1},dx=(door.x+.15)*s,dy=(door.y+.05)*s;ctx.fillStyle="#0c0d0c";ctx.fillRect(dx,dy,s*.7,s*.95);ctx.strokeStyle=accent;ctx.lineWidth=2;ctx.strokeRect(dx,dy,s*.7,s*.95);const windows=Math.max(2,Math.floor(b.w/2));for(let q=0;q<windows;q++){const wx=x+s*.65+q*(w-s*1.3)/Math.max(1,windows-1);if(Math.abs(wx-(door.x+.5)*s)<s*.8)continue;ctx.fillStyle=o.facadeStyle==="relay"?"#6bb8bd66":"#d1aa6b44";ctx.fillRect(wx-s*.28,y+h-s*.72,s*.56,s*.3);ctx.strokeStyle=accent+"aa";ctx.strokeRect(wx-s*.28,y+h-s*.72,s*.56,s*.3)}ctx.fillStyle=accent;ctx.beginPath();ctx.arc(x+w*.5,y+s*.95,s*.16,0,7);ctx.stroke();ctx.font=`${Math.max(8,s*.2)}px monospace`;ctx.textAlign="center";ctx.fillText(o.name.toUpperCase(),x+w/2,y-s*.72);ctx.textAlign="start";ctx.restore();
}
